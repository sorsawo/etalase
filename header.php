...
echo '<a class="skip-link screen-reader-text" href="#main-content">' . esc_html__( 'Skip to content', 'etalasepress' ) . '</a>';
...
if ( has_custom_logo() ) {
    ...
    $logo_tag = ( apply_filters( 'etalasepress_h1_site_title', false ) || ( is_front_page() && is_home() ) ) ? 'h1' : 'p';
    echo '<' . $logo_tag . ' class="site-title"><a href="' . esc_url( home_url() ) . '" rel="home">' . get_bloginfo( 'name' ) . '</a></' . $logo_tag . '>';

    if ( apply_filters( 'etalasepress_header_site_description', false ) ) {
